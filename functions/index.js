...
// ===============================================
// --- UPDATE: clockEmployeeOut con Gestione Niente Pausa ---
// ===============================================
exports.clockEmployeeOut = functions.region('europe-west1').https.onCall(async (data, context) => {
     ...
     if (!uid || (callerRole !== 'dipendente' && callerRole !== 'preposto')) {
       ...
       // AGGIUNTO: pauseSkipReason e note per la gestione della mancata pausa
       const { deviceId, areaId, pauseSkipReason, note } = data;
     ...
     try {
         ...
         // 🛑 BLOCCO DI SICUREZZA GEOGRAFICO 🛑
         ...
         const updateData = {
             ...
             lastModifiedBy: uid, 
             lastModifiedAt: FieldValue.serverTimestamp(),
             note: note || '' // Salva la nota (che può essere il motivo "Altro" o una nota normale)
         };

         // === LOGICA NUOVA: GESTIONE PAUSA SALTATA ===
         if (pauseSkipReason) {
             updateData.skippedBreak = true;
             updateData.skippedBreakReason = pauseSkipReason; // es. '01', '02', etc.
             updateData.skipBreakStatus = 'pending'; // In attesa di approvazione
         }
...
// ===============================================
// --- NUOVA FUNZIONE: Revisione Richiesta No-Pausa ---
// ===============================================
exports.reviewSkipBreakRequest = functions.region('europe-west1').https.onCall(async (data, context) => {
    const callerUid = context.auth?.uid;
    const callerRole = context.auth?.token.role;

    // Solo Admin e Preposti possono revisionare
    if (!callerUid || (callerRole !== 'admin' && callerRole !== 'preposto')) {
        throw new functions.https.HttpsError('permission-denied', 'Azione non permessa.');
    }

    const { timeEntryId, decision, adminId } = data;
    
    if (!timeEntryId || !decision) {
        throw new functions.https.HttpsError('invalid-argument', 'Dati mancanti.');
    }
    if (!['approved', 'rejected'].includes(decision)) {
        throw new functions.https.HttpsError('invalid-argument', 'Decisione non valida (approved/rejected).');
    }
    ...
    try {
        ...
        if (!entryDoc.exists) {
            throw new functions.https.HttpsError('not-found', 'Timbratura non trovata.');
        }

        // Aggiorna lo stato
        await entryRef.update({
            skipBreakStatus: decision,
            reviewedBy: adminId || callerUid,
            reviewedAt: FieldValue.serverTimestamp()
        });

        return { success: true, message: `Richiesta ${decision}.` };

    } catch (error) {
        console.error("Errore reviewSkipBreakRequest:", error);
        throw new functions.https.HttpsError('internal', `Errore server: ${error.message}`);
    }
});
