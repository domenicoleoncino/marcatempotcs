// File: functions/src/index.js (VERSIONE FINALE STABILE E SENZA BLOCCO DEVICE ATTIVO)
...
// ===============================================
// --- NUOVA FUNZIONE: Registrazione e Controllo Dispositivo (COMMENTATA) ---
// ===============================================
/*
exports.registerDeviceAndCheckAccess = functions
    .region('europe-west1') 
    .https.onCall(async (data, context) => {
        ...
        // ... (Logica di blocco dispositivo) ...
        return { status: 'allowed', message: 'Accesso temporaneamente permesso (Logica di blocco disattivata).' };
    });
*/
// ===============================================
    ...
    try {
        ...
        if (role === 'dipendente' || role === 'preposto') {
            const employeeData = {
                ...
                workAreaIds: [], deviceIds: [], // deviceIds vuoto in attesa del primo login
                createdAt: FieldValue.serverTimestamp()
            };
...
exports.manualClockIn = functions.region('europe-west1').https.onCall(async (data, context) => {
    ...
    if (!employeeId || !workAreaId || !timestamp || !adminId) { // RIMOZIONE: 'note' dal controllo obbligatorio
        throw new functions.https.HttpsError('invalid-argument', 'Dati obbligatori mancanti (employeeId, workAreaId, timestamp, adminId).');
    }
    ...
    try {
        ...
        await db.collection('time_entries').add({
            ...
            pauses: [],
            isManual: true, 
            note: data.note || 'N/D', // Usa la nota fornita o 'N/D' se non fornita
            createdAt: FieldValue.serverTimestamp(),
        });
...
exports.manualClockOut = functions.region('europe-west1').https.onCall(async (data, context) => {
    ...
    if (!employeeId || !timestamp || !adminId) { // RIMOZIONE: 'note' dal controllo obbligatorio
        throw new functions.https.HttpsError('invalid-argument', 'Dati obbligatori mancanti (employeeId, timestamp, adminId).');
    }
...
exports.clockEmployeeIn = functions.region('europe-west1').https.onCall(async (data, context) => {
    ...
    try {
        ...
        // Logica Registrazione/Validazione Dispositivo (Server-Side) - RIMOSSA LOGICA DI REGISTRAZIONE
        const currentDeviceIds = employeeDoc.data().deviceIds || [];
        const employeeRef = employeeDoc.ref;
        
        // La logica di registrazione è stata rimossa, ma teniamo pulito il payload che non viene usato
